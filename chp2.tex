...
create an effect or deliver a message'', which is achieved by combining various sources of sound into a programme
\citep[pp. 12, 20]{Hausman2012}.  Producing radio requires the combination of complex audio technology with artistic
taste and judgement \citep{Barbour2004}.

In this section, we shall look at what audio editing is, and how it is performed using digital audio workstations.
...
\begin{figure}[t]
  ...
  \begin{subfigure}{\textwidth}
    ...
    \caption{Zoomed-in waveform. The frequency information is visible.}
  ...
  \begin{subfigure}{\textwidth}
    ...
    \caption{Zoomed-out waveform. The frequency information is not visible.}
...
The ambition of semantic audio is to make sound less ``opaque'' by being able to understand what is contained within.

Semantic audio brings together a wide variety of disciplines, including speech recognition, information retrieval,
audio analysis, signal processing, psychoacoustics, and machine learning \citep{Foote1999}. As such, we will only aim
to provide a brief overview of the methods and applications that may be relevant to radio production.
...
works by decomposing acoustic signals into semantic entities

has applications in improving the usability of audio user interfaces

music segmention by chorus and verses for intelligent audio editing \citep{Fazekas2007}

Has applications to speech content too
For example, it can be used to distinguish between music and speech \citep{Wieser2014}, show where different people are
speaking \citep{AngueraMiro2012}, and who those people are \citep{Doddington1985}.

Semantic audio analysis has wide applications to music and an active research community, as is evident by the annual
community-driven evaluation of music information retrieval algorithms \citep{Downie2008}


% METHODS 
\subsection{Methods}

Semantic audio analysis begins with using an algorithm to process the raw audio data into semantically meaningful
``features'' or ``descriptors'', known as \textit{feature extraction}. Audio features are a numerical representation of
a certain property of the audio. For example, these might include how loud the sound is, or its estimated pitch.

Many semantic audio algorithms include a second stage, where the audio features are classified into semantically
...
% Not including music features:

%\paragraph{Rhythm}
% Onset rate
% Rhythm strength
% Mean correlation peak
% Peak/valley correlation ratio
% Beat counts
% Tempo
...
There are a huge number of semantic audio features that can be extracted from an audio signal.  In this section, we
will describe a selection of features that have applications to radio production, and are mentioned later in this
thesis. These only represent a small selection of the available features, but we have included them to give the reader
an impression of how semantic audio works.

Semantic audio features can be broadly categorized into energy, temporal and spectral features. For musical features,
we could extend this list to include rhythmic and harmonic features, but we have chosen to exclude music-only features
from this section.

\paragraph{Pre-processing}

When an audio file is processed with a feature extraction algorithm, the audio is first cut into lots of overlapping
clips, called ``frames''.  The vectors returned for each frame are then averaged, commonly by producing mean and
standard deviation measurements.

\paragraph{Energy features}

Energy features are very quick to calculate, which makes them attractive. The energy of the signal, and how it changes
over time, can go some way to helping us understand or classify the audio.

% RMS Energy
Root mean square (RMS) energy is the square root of the mean of the square of the audio signal. RMS is commonly used in
scientific work as a measurement of a signal’s power.
...
% Low energy ratio
Low energy ratio is the proportion of frames whose RMS amplitude is lower than a fixed threshold.
...
%% Intensity
%Intensity, or intensity ratio, is the sum of a signal’s FFT bins.

%% Loudness
%Loudness is calculated by dividing the signal into Bark bands, finding the ``specific loudness'' for each, and summing
%them together. 

\paragraph{Temporal features}

% Zero-crossing rate
This is the number of times the audio signal crosses the time axis per second. It is a crude measure of pitch, but is a
very quick and easy feature to calculate.
...
%% Temporal centroid
%The algorithm to calculate the temporal centroid uses the ‘signal envelope’, which is composed of the mean squared
%value of each frame.

%% Log attack time
%This feature also uses the signal envelope defined in Equation 19. The log attack time is the log of the time between
%the beginning of the signal (T0), and the maximum signal value (T1). The beginning is defined as the time at which the
%signal first exceeds 2\% of maximum value.

%% Kurtosis
%Kurtosis is a measure of the shape of the signal’s probability distribution. It if often described as a measure of the
%‘peakedness’ of the distribution.

%% Variance
%The variance of a frame is the second central moment of the samples, and is calculated as follows:

%% Skewness
%Skewness is a measure of asymmetry in the distribution of the samples in a frame. It is calculated using the variance.

%% Average deviation
%This feature measures the average deviation from the mean value of the frame.

%% Non-zero count
%This feature is simply a count of the number of samples in a frame that are not zero-valued.

%% Highest-lowest value
%These algorithms simply find the highest and lowest valued samples in the input frame.


\paragraph{Spectral features}

% Spectral centroid
Spectral centroid is often used to measure the ‘brightness’ of an audio signal. This is done by treating the spectrum
as a distribution of frequencies, with the probability of each frequency being based on its normalised amplitude.
...
%% Spectral spread
%The spectral spread is the variance of the distribution used to calculate the spectral centroid.

%% Spectral skewness
%The skewness of the spectrum is a measure of the asymmetry of the distribution used to calculate the spectral
%centroid. A distribution with perfect symmetry has a skewness of 0. Skewness to the right results in a value less than
%0, and for the left, greater than 0.

%% Spectral kurtosis
%Kurtosis is the measure of the flatness of a distribution.

%% Spectral slope
%The spectral slope represents the angle at which the spectral amplitude decreases

%% Spectral rolloff
%The spectral rolloff is the frequency below which lies 95\% of the signal energy.

%% Spectral flatness
%Spectral flatness is a measure of noisiness. A high value indicates a noisy signal. It is calculated as the ratio of
%the geometric mean to the arithmetic mean of the energy spectrum.

%spectral entropy \citep{Misra2004} is a
%measure of the peakiness of the spectrum and is an effective feature in distinguishing between voiced and unvoiced
%speech.

%% Spectral crest
%Spectral crest is the ratio between the maximum value and the arithmetic mean.

%% Intensity ratio
%The intensity ratio describes the relative intensity at various frequencies. As was done for the rhythm features, the
%FFT bins of the audio signal are divided into K sub-bands, defined by the frequencies in Equation 8, and summed to
%produce K signals (Ak). They are each then compared to the overall intensity, as defined in Equation 5, to produce an
%intensity ratio vector of length K.

% MFCC
Mel-frequency Cepstral Coefficients are a compact representation of the spectral envelope. They were developed [38]
for, and are commonly used as a speech analysis tool, but also have applications in music analysis. The coefficients
are calculated using a cepstrum (non-linear spectrum of a spectrum) of the audio signal, but with the frequency bands
spaced using the mel scale, which gives a better approximation to the human hearing system.

The audio signal is first filtered into K separate signals using a triangular filter bank. Each output signal is a sum
of the FFT bins, with each bin weighted by the ‘amplitude’ of its centre frequency in the filter bank (see Figure
12). Every filter bank implementation is slightly different, but most have linear spacing up to approx. 1kHz, with log
spacing following that. The QMUL implementation uses the following formula to calculate the boundary frequencies for
NLIN + NLOG filters.

The MFCCs are calculated using a weighted sum of the log-energy of the K signals, using the
formula below.
...
%% Bark bands
%Bark coefficients are calculated simply by summing the value of the FFT bins which lie in each Bark band.

%% Spectral flux
%Spectral flux is the sum of the change in magnitude of each frequency bin, restricted to the positive changes.

%% Spectral sharpness
%Spectral sharpness is perceptually equivalent to the spectral centroid, but is computed using the specific loudness of
%the Bark bands [37].

%% Standard deviation
%The spectral standard deviation is the square root of the spectral spread (aka. spectral variance).

%% Irregularity
%Spectral irregularity is a method of measuring noisiness. There are two varieties from Krimphoff [39] and Jensen [40].

%% Average deviation
%Average deviation is the mean deviation (aka. absolute deviation) from the spectral centroid.
...
In this section we will look at some of the applications of semantic audio analysis that are relevant to radio
production.  As most original radio productions are speech-based, and because speech production is the focus of our
work, we will not cover applications to music-only content.
...
\paragraph{Speaker identificaton}
