...
In this chapter, we start with a brief overview of radio production and describing the limitations of the current tools
that producers use to edit radio programmes.  We show how semantic audio analysis can be used to extract information
from the audio content that usefully describes the sound.  We then consider previous user interfaces that have used
this semantic data to improve the navigation and editing of audio through visualization, transcripts, and audio
playback. 
...
radio programmes from studio 2LO in London \citep{BBC2015}. In 1927, the British Broadcasting Corporation
was founded by Royal Charter, which defined the BBC's objectives, powers and obligations, and guaranteed its
independence from government.  The current charter \citep{BBCCharter2016} states that the BBC's mission is to ``act in
the public interest, serving all audiences through the provision of impartial, high-quality and distinctive output and
...
stations \citep[p.~4]{BBC2017a}.  In addition, BBC World Service Radio operates a global radio service in 29 languages,
with over 154M listeners per week \citep[p.~32]{BBC2017a}.
...
supplemented by an additional \textsterling210M from BBC Worldwide -- a wholly-owned commercial subsidary that
...
was spent on the national radio networks, 25\% of local stations and 15\% on regional services \citep{BBC2017a}.
The average cost of producing radio on the national networks in 2p per listener per hour \citep[p.~27]{BBC2017a}.
%By comparison, television production costs at least six times more to produce per hour of consumption \citep[pp. 25,
%27]{BBC2017a}.
...
Increasing demands for space and cost of accommodation in London mean that more radio content is being produced in open
plan offices. This creates challenges for audio production, such as noise, privacy and reliance on headphones for
monitoring \citep{Brixen2003}.

% technology
\subsubsection{Technology}
The BBC's radio production activities are centrally operated through a radio production system called \textit{dira!
Radio}, made by SCISYS \citep{SCISYS2015}. This system handles audio recording, storage management, editing, playout
and scheduling. \textit{dira!} integrates with a range of other specialist systems for distribution, advanced audio
editing, metadata publication, commissioning and rights management.
...
editing.  SADiE has much more advanced functionality for controlling EQ and effects, and for handling bigger projects.
Other audio editing systems are also used, such as Adobe Audition, but these are not sanctioned by the BBC.
...
that speech has freqent silent gaps between words, wheras music does not. The threshold can be set as a fixed value
