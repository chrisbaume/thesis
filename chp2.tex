...
so a low value indicates the presence of mostly low-frequency sound and vice-versa.
...
which is a crude measure of the primary frequency of the audio.
...
Waveforms and spectrograms provide a direct mapping of sound to vision, which is then be interpreted by the user. The
benefit of this approach is that it is flexible, as it can be used for a wide variety of content such as
speech, music or noise. The downside is that the user must learn how to read these visualizations, and view them at an
appropriate scale.
...
categories. These have previously been combined with data visualization techniques \citep{Aigner2011} to create novel
user interfaces that aid the navigation and editing of audio content.

We have divided these segmentation techniques into two categories: classification, where audio is segmented by the
nature of the audio, and topic summarization, where the audio is segmented by its contents.

\subsubsection{Classification}

Audio taxonomy can be used to segment audio into categories within a taxonomy. This has applications to audio editing
\cite{Avdelidis2007}
...
\citet{Foote1998} creater a video browser that used an adjustible slider to set a threshold for the display of speaker
diarization segments


% TOPIC SEGMENTATION
\subsubsection{Topic summarisation}
