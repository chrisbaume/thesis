...
editing using a pen-based interface compared to a screen-based interface.  We found that neither interface was best
overall, but that each was better for certain things in different situations.  Broadly speaking, we found that our pen
interface was better suited for making simple edits involving quick decisions, where the producer is familiar with the
content and has a high-quality transcript, when they are working with others in the same room or away from the desk.
By contrast, we found that our screen interface was better suited to more complicated editing involving complex
decisions, where the producer is less familiar with the content and has a lower accuracy transcript, when they are
working with people remotely or at their desk.

The main difference between the pen and screen interfaces was that the screen included integrated playback and
correction features, whereas the pen did not.  Participants reported that the simplicity of the pen interface, in
combination with the benefits of using paper, was faster and made it easier to make decisions. However, the more
feature-rich screen interface was reported to be better for more in-depth, complicated editing.

Due to technical limitations, the design of our paper interface involved generating edit commands based on marks in
rectangular boxes. There were interpreted very literally, which created a potential source of errors, and forced the
participants to draw carefully when editing. Humans are not good at drawing straight lines in freehand, so it would be
better to create a system that was more forgiving in this respect.
