...
In order to build our prototype, we needed to design the layout of the document and define a set of gestures for
editing the content.  As there were no previous systems on which to base our design, this process raised a number of
questions about what information to include in the layout, and which gestures should be used for interaction.
We formulated the following three questions that we wanted to explore:
...
To answer these questions, we created a paper prototype of our paper interface. The prototype used a normal pen rather
than a digital pen, so did not edit the audio. However, the prototype allowed us to test an initial design of our
interface with users before building the functional system.
In this section, we describe the design of our paper prototype, explain how we evaluated it and outline our findings.

\subsection{Paper prototype design}

% Transcript, from speech-to-text system
The basis of our inital design was a printed transcript. The transcript was generated using an automated speech-to-text
system. We chose to use this rather than a perfect transcript as the errors contained within the automated transcript
may affect user behaviour.

% Limitations of Anoto system, used normal pen for prototype
The Anoto Live Forms platform, which we used to create our system, works by dividing the page into rectangular zones.
When a digital pen draws inside one of these zones, that data can then be captured digitally and processed.
Traditionally this is used to detect when someone has ticked a box on a form, or to extract an image of a signature
drawn inside of a box. However, by creating zones that align with the text of a transcript, we would be able to use
this system to digital capture editing annotations.

Having to divide the page into rectangular zones imposes some limitations on the design.

% Edit commands: selector beneath word (double-spaced) and at end of line, removal by crossing word
From what we had witnessed from previous experiments, the most common annotations used by producers were underline,
strikethrough and drawing a line down the side of the page.
Double-spaced to leave enough room for underline.
The transcript was printed double-spaced, with a grey box below each word to indicate the underlining area.
Each line of the transcript had a timestamp on the left and a square box on the right, which
...
The speech-to-text system provided additional information about the speech.
Speaker diarization
Confidence shading information provided by speech-to-text
Timestamps
Words with
a low confidence rating were `low-lighted' by shading them grey -- a technique known as confidence
shading \citep{Vemuri2004}.
