...
% Mechanisms - digital ink, digital pen, barcode
In this section, we have seen how barcodes, digital ink and digital pens have been used to link written documents to
timed media.

Barcodes provide a simple mechanism for linking physical printed paper to digital media. They are easy and free to
generate, and are robust to photocopying.

A device is needed to read the barcodes, but this could be done using a mobile phone.
However, barcodes only provide a one-way link from paper to media. It would be possible to implement a feedback
mechanism, but the data would have to be entered into the scanning device rather than the paper. 
...
segment the transcript into paragraphs, with the start of each paragraph labelled with the speaker identifier in square
