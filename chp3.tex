...
% mapping the feature
We coloured the waveform by mapping the low energy ratio to a gradient between two easily distinguished colours.
We used blue to represent speech and pink to represent music.
We chose blue, to match the waveform colour of the BBC's radio production system, and its inverse colour pink.
...
\begin{enumerate}[label=H\arabic*.]
  \item \textbf{Effort}: The audio visualisation affects the effort required to complete the task, in decreasing order
    of C1, C2 and C3.
  \item \textbf{Time}: The audio visualisation affects the time taken to complete the task, in decreasing order of C1,
    C2 and C3.
  \item \textbf{Accuracy}: The audio visualisation affects the accuracy of the result, in increasing order of C1, C2
    and C3.
...
% reported TLX
To measure effort, we used both qualitative and quantitative data.

% Time
% first action to task completion
...
\begin{subfigure}{.5\textwidth}
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      ytick=data,
      xlabel=Participants,
      symbolic y coords={Professional,Consumer,None},
    ]
    \addplot[fill=black!20] coordinates
      {(29,Professional) (6,Consumer) (6,None)};
    \addplot[fill=white] coordinates
      {(17,Professional) (5,Consumer) (0,None)};
...
\hline
& \textbf{Audio vs Waveform} & \textbf{Audio vs Coloured} & \textbf{Waveform vs Coloured} \\ \hline
	Seek actions & \highsig$<0.01$ & \highsig$<0.01$ & \highsig$<0.01$ \\
	Error & \nosig$>0.05$ & \highsig$<0.01$ & \medsig$<0.05$ \\
	Time & \nosig$>0.05$ & \highsig$<0.01$ & \highsig$<0.01$ \\
	TLX Effort & \medsig$<0.05$ & \highsig$<0.01$ & \highsig$<0.01$ \\
