...
\hline
  & Audio vs Waveform & Audio vs Coloured & Waveform vs Coloured \\ \hline
	Seek time & \highsig$<0.01$ & \highsig$<0.01$ & \highsig$<0.01$ \\ \hline
...
\end{tabular}}
\caption{$p$-value of pairwise comparisons}
\end{table}
...
%, including seek actions, selections, zoom and play/pause.
...
%To ensure that participants followed instructions correctly, their results were filtered using an acceptance criteria.
%Any participant who submitted a response with an absolute error of more than 5 seconds was rejected.
...
This section describes the various tools that were used to analyse the data collected using the experiment, find
differences between conditions and determine whether they are of significance.
...
A clear majority of 76\% thought that the colourised waveform was the easiest to use, with the normal waveform receiving
