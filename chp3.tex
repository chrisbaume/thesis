...
information, users cannot see the spectrum or timbre of the audio.  This potentially impacts on the ability of users to
efficiently navigate and edit audio content. However, we were unable to find any studies that have attempted to measure
the effectiveness of waveforms for navigating or editing audio content.

% waveforms are used in radio production
%The software radio producers use to navigate and edit audio content visualises the audio using waveforms.
...
By using an audio visualisation that can present more information, it is
possible that this would give radio producers a more powerful tool to interact with their audio content.
...
\citet{Tzanetakis2000,Rice2005,Mason2007} all developed methods of mapping semantic information to waveform colours, but did not perform
a user evaluation.
Waveforms are often used as a baseline by which to measure new visualisation techniques.
However, there is little evidence about the performance of waveforms themselves.  As such, we were interested in
studying how audio waveforms affect performance of audio editing tasks.

% semantic audio visualisations have not been compared to waveforms and not tested in radio production tasks
We saw in Chapter~\ref{chp:background} how previous work has mapped semantic audio features to visualisations to aid
navigation of audio content. These techniques could potentially provide applications to radio production and give
producers better tools for interacting with their content.  However, we could not find any studies that have attempted
to measure the impact of these visualisation techniques on audio production.  As such, we were interested in exploring
whether semantic audio visualisation techniques had an impact on radio production.  To do this, we conducted a study in
which pariticipants used different audio visualisations to perform a common radio production task, and measured their
performance.
...
Waveforms are prevalent in radio production, so removing the audio waveform entirely may be disruptive for users.
Adding colour to waveforms would allow us to include additional semantic information about the audio, while being able
to retain a familiar visualisation that users are comfortable with and can already read.  Therefore, rather than aiming
to replace the waveform, we were interested in seeing whether mapping a relevant semantic audio feature to the colour
would impact on the performance of the user's task.

% The task
\subsection{Task}
For the task, we chose segmentation of music from speech.  We wanted the task to be a common radio production activity,
but one that is simple enough that it can be performed by anybody.  In radio production, music has to be edited out
when turning an off-air programme recording into a podcast. This is due to music licencing issues, which are different
for downloadable content than they are for radio broadcasts. The music is removed with a DAW that uses a waveform to
visualise the audio. Although the waveform can be used to distinguish between music and speech, at typical zoom levels
it is not obvious. This means that removing the music can be a slow and tedious process.
...
seek actions (i.e. clicks on the visualisation) used to complete the task.
...
each visualisation.  For this, we used the NASA Task Load Index, or `NASA-TLX', \citep{Hart1988} questions, listed
...
To measure effort, we used the effort ratings; to measure time, we used the temporal demand ratings; and to measure
accuracy, we used the performance ratings.

We used the `Raw TLX' values by excluding the second part of the measurement in which the subscales are converted into
a single value by weighting them by importance \citep{Hart2006}.  We did this to reduce the time required for data
collection, and to be able to analyse each subscale individually.
...
The interface displayed the overall visualisation as well as a zoomed in view above it. The participant could navigate
the audio by clicking on either view, which would seek to that position in the audio.  Buttons below the visualisation
controlled play/pause, zoom level and setting the in-- and out-points of the selection.  The selection was displayed by
highlighting the visualisation, and on a slider below it. The selection could be adjusted by dragging either end of the
...
columns from our $18\times9$ Latin square and mapping the values 1--3, 4--6 and 7--9 to 1, 2 and 3, respectively.  By
calculating the carryover effect of each column of the $18\times9$ matrix, we found that the middle three columns had
the minimum carryover effect, so we used them for our visualisation sequence.
...
We used SPSS to conduct a
repeated measures multivariate ANOVA to test for statistically significant differences in the task load ratings. We
then used Tukey's honest significant difference (HSD) post-hoc test to make pairwise comparisons between the
visualisation for each metric.

We couldn't re-use audio clips for the tasks, so each participant only experienced a subset of the combinations of
visualisations and audio clips. This resulted in an incomplete dataset, which prevented us from using from using
repeated measures ANOVA. Instead, we used a mixed model as it can account for incomplete data and repeated measures
design.

We used SPSS to perform a linear mixed effects analysis of the relationship between visualisation and the three
performance metrics (seek actions, task completion time and task error).  We configured the visualisations as a fixed
...
We analysed the performance metrics using a linear mixed model.  Our model found statistically significant differences
between the three conditions for seek actions, task time, and task error.  Figure~\ref{fig:visualisation-metrics} shows
...
[$F(3,366)=93.871, p<.00001$]. On average, the enhanced waveform (C3) required 7.5 (30\%) fewer seek actions than the
normal waveform (C2), and 12.7 (43\%) fewer than having no visualisation (C1). Additionally, the normal waveform (C2)
required 5.2 (17\%) fewer seek actions than having no visualisation (C1).  The differences in seek actions between all
three conditions were statistically significant ($p<.01$). These results confirm hypothesis H1 (effort).
...
p<.00001$].  On average, the task was completed 9 seconds (13\%) faster using the enhanced waveform (C3) compared to
the normal waveform (C2) ($p<.01$), and 10.9 seconds (15\%) faster compared to having no visualisation (C1) ($p<.01$).
There was no statistically significant difference in task completion time between the normal waveform (C2) and no
visualisation (C1).  The mean time of the normal waveform was only 1.9 seconds (3\%) faster than no visualisation.
These results suggest that hypothesis H2 (time) is confirmed for C2$>$C3, but not for C1$>$C2.
...
average, the error when using the enhanced waveform (C3) was 87ms (14\%) lower than when using the normal waveform (C2)
($p<.05$), and 122ms (19\%) lower than when there was no visualisation (C1) ($p<.01$). There was no statistically
significant difference in task error between the normal waveform (C2) and no visualisation (C1).  The error using the
normal waveform was only 35ms (5\%) less than with no visualisation.  These results suggest that hypothesis H3
(accuracy) is confirmed for C2$<$C3, but not for C1$<$C2.
...
The task load ratings from participants show that compared to both the normal waveform (C2) and no visualisation (C1),
the enhanced waveform (C3) was less mentally and physically demanding, had better performance, was less frustrating and
required less effort.  They also rated the enhanced waveform as less temporally demanding than the normal waveform.
Compared to having no visualisation (C1), participants rated the normal waveform (C2) as being less frustrating and
...
The results are shown in Figure~\ref{fig:visualisation-comparison}.  The enhanced waveform condition was rated as the
easiest to use by 76\% of the participants.  Having no visualisation was rated as the most frustrating condition by
two-thirds of the participants. The normal waveform was not considered by many to be the easiest, nor the most
frustrating.
...
Our study found that using an enhanced waveform visualisation, participants could segment music from speech faster,
more accurately and with less effort than using a normal waveform.  When using the normal waveform, participants could
segment music from speech with less effort than having no visualisation, but we couldn't find any significant
...
Our findings are supported by both quantitative and qualitative metrics.
...
% low energy ratio is represented by waveform, but not at realistic use case
Music and speech have very different properties and are easily distinguished through listening.  The low energy ratio
feature we used is based on detecting changes in the amplitude profile. At the right resolution, these changes can
clearly be seen in an audio waveform. Although all of the information is represented using the waveform, in most cases
the waveform will have to be zoomed-out, so this information is no longer visible.  It is therefore important to not only
include the right information, but to present it in a way that humans can interpret.

There were 41 participants in our study, and most had experience of using professional audio editing software. 

Although it would have been preferable to use professional radio producers for our study, most of the participants had
experience with using professional audio editing software.


approximately 30\% fewer seek actions, 13\% faster and with 14\% more accuracy than using a normal waveform.

% Enhanced waveform was significantly better than normal waveform in all collected metrics
% Provides strong evidence that using semantic audio features in visualisations can positively affect task performance

Despite there being 41 participants in the study, there was no significant difference in the time nor accuracy of the
task when using the normal waveform compared to having no visualisation.
Although in this case, the waveform didn't appear to help participants complete the task any faster than just listening
to the audio, waveforms are still widely used for audio production.

Waveforms are used for all sorts of audio editing tasks, and this study only considered a single task. However, for
this task, the task was not completed much faster using waveforms rather than just listening to the audio.
...
In this study, we only considered a single audio editing task, and we based our enhanced visualisation on a semantic
audio feature that was specific to that task. Therefore, we can't infer anything about audio visualisations in general.
Although our enhanced waveform improved the performance of the task, it is unlikely that it would help with unrelated
tasks. This means that a custom semantic audio feature and mapping would have to be generated for each different task.

% specific vs general visualisations
The direction of future research into audio visualisation could either target specificity, where individual
visualisations are created to target particular applications, or generality, where a single visualisation attempts to
handle multiple applications. It would likely be easier to create effective specific visualisations as they only need
to include information relevant to the application, but this would make it difficult to cater for every possible use
case.  General visualisations would have to contain information relevant to all applications. It would be difficult to
select the right information and map it in a way that humans can easily interpret. However, they could be used for any
purpose without users having to switch between or learn multiple visualisations.

\section{Conclusion}

We studied the performance of audio waveforms for segmenting music in speech. 

Compared to having no visualisation, the normal waveform required fewer seek actions and was rated as needing less
effort and being less frustrating. However, we could find no significant differences in the time required to complete
the task, nor the accuracy of the result.

Compared to the normal waveform, the enhanced waveform allowed participants to 
