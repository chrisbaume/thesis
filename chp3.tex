...
\section{Further work}\label{sec:vis-further}

% only applicable to one use case
In this study, we only considered a single audio editing task, and based our enhanced visualization on an audio feature
specific to that task. Based on these results, we can't infer anything about the performance of these audio
visualizations beyond speech/music discrimination.  Our enhanced waveform improved the performance of participants in
completing the task, but it is unlikely that it would improve the performance of other editing tasks.
%This means that a custom semantic audio feature and mapping would have to be generated for each different task.
