...
paper and digital documents. PADD \cite{Guimbretiere2003} was a concept for a
system of editing documents that allowed users to move from digital
to paper and back again. ProofRite \cite{Conroy2004} was an
...
PaperProof \cite{Weibel2008} improved on this by interpreting the edit
...
that position. Video Paper \cite{Hull2003} was a system that embedded video
...
on the device. Books with Voices \cite{Klemmer2003} was a similar system which
tested this approach with oral historians who found it had substantial benefits
with minimal overhead. Erol et. al. \cite{Erol2007} went a step further by
embedding the video data in the barcode, removing the need for the server.
HotPaper \cite{Erol2008} removed the need for barcodes by using a camera to
...
annotating and editing video content.  Marquee \cite{Weher1994} synchronised
handwritten notes with a live video recording by using a horizontal line
gesture to mark a timestamp.  Videotater \cite{Diakopoulos2006} was another
...
could be written over a clip. WaCTool \cite{Cattelan2008} extended this
...
and end of an unwanted region.  Video as Ink \cite{Cabral2016} allows users to
`paint' video frames onto the tablet interface and then edit the video by using
an `eraser mode' to remove unwanted frames.

Dynomite \cite{Wilcox1997} was a note-taking system that recorded audio
...
the highlighted regions.  The Audio Notebook \cite{Stifelman2001} was a similar
...
the page.  ChronoVis \cite{Fouse2011} used the Anoto dot pattern to record
